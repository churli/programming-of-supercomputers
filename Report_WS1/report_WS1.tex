...
\begin{enumerate}
	...
	\begin{figure}[h!] % h=here, t=top, b=bottom, p=(extra)page, !=force
	 	\begin{center}
	 		\includegraphics[width=.7\linewidth]{OMP_ICC_speedup.png} % It searches in the Figures/ folder!
...
\begin{enumerate}
	...
	Strong scaling: We ran 2 experiments, one with -s 15 and one with -s 30. We see linear scaling in both cases. However, for the reduced domain size, we see that the speedup decreases with 5 OpenMP threads. This is expected because, per Amdahl's law, the maximum speedup is limited by problem size. At 5 OpenMP threads, the costs of parallelization (
	}
	\item{How does the performance compare to the results achieved with OpenMP in Section 5.1
	and with MPI in Section 5.2?
	We see linear scaling with increased number of OpenMP threads 
