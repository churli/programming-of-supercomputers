...
int main(int argc, char **argv) {
  ...
  if (rank == 0) {
    for (i = 1; i < size; i++) {
      MPI_Send((matrix_1D_mapped + (i * (local_block_size * rows))),
               (local_block_size * rows), MPI_DOUBLE, i, 0, MPI_COMM_WORLD);
  ...
  if(local_block_size <= 16){
    chunks = 1;
  }
  else{
    chunks = local_block_size/16;
  }

  assert(local_block_size%chunks == 0);

  int chunk_size = local_block_size/chunks;
  ...
  for (process = 0; process < rank; process++) {
    ...
    for(chunk_num = 0; chunk_num < chunks-1; chunk_num++){
	    for (row = chunk_num*chunk_size; row < (chunk_num+1)*chunk_size; row++) {
  ...
  for (chunk_num = 1; chunk_num < chunks; chunk_num++){
  	if (rank + 1 < size) {
	      mpi_start = MPI_Wtime();
	      MPI_Group group;
