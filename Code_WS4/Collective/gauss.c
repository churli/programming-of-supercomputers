...
int main(int argc, char** argv) {
	...
	for(process = 0; process < rank; process++) {
		...
		mpi_start = MPI_Wtime();
		MPI_Ibcast(pivots, rows+1, MPI_DOUBLE, 0, groupComm, &groupReq);
		MPI_Wait(&groupReq, MPI_STATUS_IGNORE);
		mpi_time += MPI_Wtime() - mpi_start;

		for(row = 0; row < local_block_size - 1; row++){
			mpi_start = MPI_Wtime();
			MPI_Ibcast(pivots + (rows+1)*(row+1), rows+1, MPI_DOUBLE, 0, groupComm, &groupReq);
			...
			for (i = 0; i < local_block_size; i++){
				...
				for (column = column_pivot; column < columns; column++){
					matrix_local_block[index + column] -=  tmp * pivots[(row * (rows+1)) + column + 1];
				}
	...
	for(row = 0; row < local_block_size; row++){
		...
		for (column = column_pivot; column < columns; column++){
			...
			pivots[(rows+1) * row + column + 1] = matrix_local_block[index + column];
		}

		local_work_buffer[row] = (rhs_local_block[row])/pivot;
		pivots[(rows+1)*row] =  local_work_buffer[row];
		mpi_start = MPI_Wtime();
		MPI_Ibcast(pivots + (rows+1)*row, rows+1, MPI_DOUBLE, 0, groupComm, &groupReq);
		for (i = (row + 1); i < local_block_size; i++) {
			...
			for (column = column_pivot+1; column < columns; column++){
				matrix_local_block[i*rows+column] -=  tmp * pivots[(rows+1) * row + column + 1];
			}
